...
int main(int argc, char** argv)
{
  ...
  case 2:
  {
	  ...
	  for (int i = 0; i < N_BOARDS; i++)
		  src_image[i] = imread("test_" + to_string(i) + ".jpg");
	  ...
	  for (int i = 0; i < N_BOARDS; i++) {
		 ...
		 // cvtColor(src_image[i], gray_image, CV_BGR2GRAY);
		  //cvFindCornerSubPix(gray_image, imageCorners.Size(9, 9), Size(-1, -1), TermCriteria(CV_TERMCRIT_ITER + CV_TERMCRIT_EPS, 30, 0.1));
