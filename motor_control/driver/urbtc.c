...
//#include <linux/autoconf.h>			//chg t.miyo //chg akihito
...
static int urbtc_open(struct inode *inode, struct file *file)
{
	...
	if (!interface) {
		pr_err("%s - error, can't find device for minor %d",__FUNCTION__, subminor); //chg akihito
		retval = -ENODEV;
		goto exit;
	}
...
static void urbtc_read_bulk_callback(struct urb *urb)			//chg t.miyo
{
	...
	if (urb->status && 
		...
		pr_debug("%s - nonzero read bulk status received: %d",__FUNCTION__, urb->status); //chg akihito
		dev->readbuf_enabled = 0;
		return;
	}
...
static void urbtc_write_bulk_callback(struct urb *urb)			//chg t.miyo
{
	...
	if (urb->status && 
	    !(urb->status == -ENOENT || 
	      urb->status == -ECONNRESET ||
	      urb->status == -ESHUTDOWN)) {
		pr_debug("%s - nonzero write bulk status received: %d",__FUNCTION__, urb->status); //chg akihito
	}
...
static ssize_t urbtc_write_async(struct file *file, const char *user_buffer, size_t count, loff_t *ppos)
{
	...
	if (retval) {
		pr_err("%s - failed submitting write urb, error %d", __FUNCTION__, retval); //chg akihito
		goto error;
	}
...
static int urbtc_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	if (dev == NULL) {
		pr_err("Out of memory"); //chg akihito
		goto error;
	}
	...
	for (i = 0; i < iface_desc->desc.bNumEndpoints; ++i) {
		...
		pr_debug("endpoint %d(%d) %s",
		    endpoint->bEndpointAddress,
		    endpoint->bEndpointAddress & ~USB_ENDPOINT_DIR_MASK,
		    endpoint->bEndpointAddress & USB_ENDPOINT_DIR_MASK? "in": "out"); //chg akihito
		if (((endpoint->bEndpointAddress & USB_ENDPOINT_DIR_MASK)
			...
			case EP_READREQ:
				...
				if (!dev->readreq_buffer) {
					pr_err("Could not allocate readreq_buffer"); //chg akihito
					goto error;
				}
				break;
			case EP_READ:
				...
				if (!dev->readbuf_urb) {
					pr_err("Could not allocate readbuf_urb"); //chg akihito
					goto error;
				}
				dev->readbuf_size = buffer_size;
				dev->readbuf_work = kmalloc(buffer_size, GFP_KERNEL);
				if (!dev->readbuf_work) {
					pr_err("Could not allocate readbuf_work"); //chg akihito
					goto error;
				}
				dev->readbuf_buffered = kmalloc(buffer_size, GFP_KERNEL);
				if (!dev->readbuf_buffered) {
					pr_err("Could not allocate readbuf_buffer"); //chg akihito
					goto error;
				}
				usb_fill_bulk_urb(dev->readbuf_urb, dev->udev,
		...
		if (((endpoint->bEndpointAddress & USB_ENDPOINT_DIR_MASK)
			...
			case EP_CCMD:
				...
				if (!dev->write_counter_buffer) {
					pr_err("Could not allocate write_counter_buffer"); //chg akihito
					goto error;
				}
				break;
			case EP_SCMD:
	...
	if (retval) {
		...
		pr_err("Not able to get a minor for this device."); //chg akihito
		usb_set_intfdata(interface, NULL);
		goto error;
	}

	/* let the user know what node this device is now attached to */
	pr_info("USB Robot device now attached to urbtc - %d", interface->minor); //chg akihito
	//chg t.miyo //chg akihito
...
static void urbtc_disconnect(struct usb_interface *interface)
{
	...
	pr_info("USB Robot #%d now disconnected", minor);
	//chg t.miyo //chg akihito
...
static int __init usb_urbtc_init(void)
{
	...
	if (result) {
		pr_err("usb_register failed. Error number %d", result); //chg akihito
	}
